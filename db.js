...
DB.prototype.addDocument = function(coll, document) {
	...
	var _this=this;
	console.log("coll: " + coll + "\tdocument: " + JSON.stringify(document));
...
DB.prototype.find = function(coll, query) {
	...
	// Returns the results of the query on the given collection.
...
//									json += ',';
//							}
							var json = '{"results":' + JSON.stringify(results) + '}';
//							console.log(json + "\n");
//						}
...
DB.prototype.setNewFields = function(coll, query, object) {
	
	// Return a promise that either resolves or is rejected with the error
	// received from the database. "object" is used to store the keys and values
	// to be set.
	...
	return new Promise(function (resolve, reject) {
		_this.db.collection(coll, {strict:false}, function(error, collection){
			if (error) {
				...
				console.log("collection.update(" + JSON.stringify(query) + ", {$set: " + JSON.stringify(object) + "})");
				collection.update(query, {$set: object})
				.then(
					function() {
						console.log("Inserted new fields.");
						resolve();
					},
					function(err) {
						console.log("Failed to set new fields: " + err.message);
						reject(err.message);
					}
				)
			}
